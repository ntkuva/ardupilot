...
load("\\wsl$\Ubuntu\home\ntk\ardupilot\logs\00000003.BIN-149633.mat")
...
lgnd = {}; lgndIdx = 1;
plot(XKF5_0(:,1), XKF5_0(:,8), "g.-", "LineWidth", 1.5); %core 0 alt
lgnd{lgndIdx} = "core 0 alt"; lgndIdx = lgndIdx + 1;
if exist('XKF5_1')
    plot(XKF5_1(:,1), XKF5_1(:,8), "c.-", "LineWidth", 1.5); %core 1 alt
...
plot(BARO_0(:,1), BARO_0(:,4), "r-");
lgnd{lgndIdx} = "baro 0 alt"; lgndIdx = lgndIdx + 1;
plot(BARO_1(:,1), BARO_1(:,4), "m-");
...
plot(XKF4_0(:,1), XKF4_0(:,16), "k-");
legend("active core")

% plot which core is using each barometer
ax3 = nexttile([1 1]); hold on; grid on;
plot(XKFS_0(:,1), XKFS_0(:,5), "g.-", "LineWidth", 4);
plot(XKFS_1(:,1), XKFS_1(:,5), "b.-", "LineWidth", 2);
legend("core 0 baro", "core 1 baro");

% plot which core is using each barometer
ax4 = nexttile([1 1]); hold on; grid on;
plot(BARO_0(:,1), BARO_0(:,11), "g.-", "LineWidth", 4);
plot(BARO_1(:,1), BARO_1(:,11), "b.-", "LineWidth", 2);
