...
load("\\wsl$\Ubuntu\home\ntk\ardupilot\logs\00000004.BIN-148147.mat")
...
lgnd = {}; lgndIdx = 1;
plot(timeToSec(XKF5_0(:,2)), XKF5_0(:,8), "g.-", "LineWidth", 1.5); %core 0 alt
lgnd{lgndIdx} = "core 0 alt"; lgndIdx = lgndIdx + 1;
if exist('XKF5_1')
    plot(timeToSec(XKF5_1(:,2)), XKF5_1(:,8), "c.-", "LineWidth", 1.5); %core 1 alt
...
plot(timeToSec(BARO_0(:,2)), BARO_0(:,4), "r-");
lgnd{lgndIdx} = "baro 0 alt"; lgndIdx = lgndIdx + 1;
plot(timeToSec(BARO_1(:,2)), BARO_1(:,4), "m-");
...
plot(timeToSec(XKF4_0(:,2)), XKF4_0(:,16), "k-");
legend("active core")

% plot which core is using each barometer
ax3 = nexttile([1 1]); hold on; grid on;
plot(timeToSec(XKFS_0(:,2)), XKFS_0(:,5), "g.-", "LineWidth", 4);
plot(timeToSec(XKFS_1(:,2)), XKFS_1(:,5), "b.-", "LineWidth", 2);
legend("core 0 baro", "core 1 baro");

% plot which core is using each barometer
ax4 = nexttile([1 1]); hold on; grid on;
plot(timeToSec(BARO_0(:,2)), BARO_0(:,11), "g.-", "LineWidth", 4);
plot(timeToSec(BARO_1(:,2)), BARO_1(:,11), "b.-", "LineWidth", 2);
legend("baro 0 health", "baro 1 health");

linkaxes([ax1 ax2 ax3 ax4],'x')


function [time] = timeToSec(timeUS)
    time = timeUS./1000000;
end